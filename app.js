...
const ICONS = {"Store": "🏪", "User": "👤", "Lock": "🔒", "Users": "👥", "Package": "📦", "BarChart3": "📊", "LogOut": "🚪", "Plus": "➕", "Trash2": "🗑", "Pencil": "✏️", "CheckCircle2": "✅", "XCircle": "❌", "Clock": "🕐", "ChevronLeft": "‹", "AlertCircle": "⚠️", "KeyRound": "🔑", "Check": "✓", "X": "✕", "Loader2": "⏳", "Search": "🔍", "Camera": "📷", "MessageCircle": "💬", "Truck": "🚚", "MapPin": "📍", "Banknote": "💵", "Smartphone": "📱", "RotateCcw": "↺", "Wallet": "👛", "RefreshCw": "🔄", "Bell": "🔔", "Settings": "⚙️", "Send": "📤", "Tag": "🏷"};
      ...
      <div className="space-y-1.5">
        {rows.map((r) => (
          <div key={r.id} className="flex gap-1.5 items-center">
            ...
            <input value={r.label} onChange={(e) => updateRow(r.id, "label", e.target.value)} placeholder="مثال: من 10 قطع (اختياري)" className="field-input rounded-lg px-2 py-1.5 text-xs flex-1" />
            {rows.length > 1 && (
...
function makeEmptyNewProduct() {
  return { name: "", image: null, costPrice: "", categoryId: null, retailRows: [makeEmptyRow()], halfRows: [makeEmptyRow()], wholesaleRows: [makeEmptyRow()] };
}
...
function PricesScreen({ user, products, setProducts, productsLoading, changedToday, setChangedToday, categories, setCategories, setView }) {
  ...
  const startEdit = (p) => {
    setEditingId(p.id);
    setEditError("");
    setDraft({ retailRows: toEditRows(p.retail), halfRows: toEditRows(p.half), wholesaleRows: toEditRows(p.wholesale), costPrice: p.costPrice ?? "" });
  };
  ...
  const finalizeAddProduct = (overwriteId) => {
    ...
    if (overwriteId) {
      ...
      const updated = { ...existing, name: newProd.name.trim(), retail, half, wholesale, image, categoryId: newProd.categoryId, updatedAt: Date.now(), ...(isAdmin ? { costPrice } : {}) };
      ...
      const newId = uid();
      const newProduct = { id: newId, name: newProd.name.trim(), retail, half, wholesale, image: newProd.image || null, costPrice, categoryId: newProd.categoryId, createdAt: Date.now(), updatedAt: Date.now() };
  ...
  return (
    <div className="shop-root">
      ...
      <div className="max-w-lg mx-auto px-4 py-4 fade-up space-y-6">
        ...
        <section>
          <h2 className="font-bold text-sm text-sky-400 mb-3">المfunction notifyStoreError(collectionName, detail) {
  try { window.dispatchEvent(new CustomEvent("store-error", { detail: { collectionName, detail } })); } catch {}
}

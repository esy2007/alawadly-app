  ...
  useEffect(() => {
    ...
    qr.start(
      ...
      (decodedText) => {
        if (stopped) return;
        stopped = true;
        qr.stop().catch(() => {}).finally(() => onDetected(decodedText));
      },
      () => {}
...
function makeEmptyNewProduct() {
  return { name: "", image: null, barcode: "", costPrice: "", categoryId: null, retailRows: [makeEmptyRow()], halfRows: [makeEmptyRow()], wholesaleRows: [makeEmptyRow()] };
}
...
function PricesScreen({ user, products, setProducts, productsLoading, changedToday, setChangedToday, categories, setCategories, setView }) {
  ...
  const handleScanResult = (code) => {
    if (scannerTarget === "new") {
      setNewProd((v) => ({ ...v, barcode: code }));
    } else if (scannerTarget === "edit") {
      setDraft((v) => ({ ...v, barcode: code }));
    } else if (scannerTarget === "lookup") {
  ...
  const handleRefresh = async () => {
    ...
    if (fresh) {
      setProducts(fresh);
      showToast("تم تحديث الأسعار لآخر نسخة محفوظة");
    } else {
      showToast("تعذر الاتصال، الأسعار المعروضة هي آخر نسخة عندك");
    }
  };
  ...
  const startEdit = (p) => {
    setEditingId(p.id);
    setEditError("");
    setDraft({ retailRows: toEditRows(p.retail), halfRows: toEditRows(p.half), wholesaleRows: toEditRows(p.wholesale), costPrice: p.costPrice ?? "", barcode: p.barcode ?? "" });
  };
  ...
  const finalizeAddProduct = (overwriteId) => {
    ...
    if (overwriteId) {
      ...
      const updated = { ...existing, name: newProd.name.trim(), retail, half, wholesale, image, barcode: newProd.barcode ? newProd.barcode.trim() : null, categoryId: newProd.categoryId, updatedAt: Date.now(), ...(isAdmin ? { costPrice } : {}) };
      ...
      const newId = uid();
      const newProduct = { id: newId, name: newProd.name.trim(), retail, half, wholesale, image: newProd.image || null, barcode: newProd.barcode ? newProd.barcode.trim() : null, costPrice, categoryId: newProd.categoryId, createdAt: Date.now(), updatedAt: Date.now() };
    ...
    <div className="shop-root">
      ...
      <div className="max-w-lg mx-auto px-4 py-2 fade-up">
        ...
        <div className="flex gap-2 mb-3">
          ...
          <button onClick={handleRefresh} disabled={refreshing} title="تحديث الأسعار" className="icon-btn rounded-xl px-3">
            <Icon name="RefreshCw" size={18} className={refreshing ? "animate-spin" : ""} />
          </button>
          <button onClick={() => setScannerTarget("lookup")} title="امسح الباركود" className="icon-btn rounded-xl px-3">
              ...
              <div key={p.id} className="panel p-4 rounded-2xl relative">
                ...
                {editing && (
                  <div className="flex gap-2 mt-2">
                    <input value={draft.barcode} onChange={(e) => setDraft({ ...draft, barcode: e.target.value })} placeholder="الباركود (اختياري)" className="field-input flex-1 rounded-md px-2 py-1.5 text-xs" />
                    <button type="button" onClick={() => setScannerTarget("edit")} className="icon-btn rounded-md px-2.5"><Icon name="ScanLine" size={15} /></button>
                  </div>
                )}
        ...
        {showAdd && (
          <Modal title="➕ إضافة منتج جديد" accent="#34D399" onClose={() => setShowAdd(false)}>
            ...
            <div className="flex gap-2 mb-3">
              <input placeholder="الباركود (اختياري)" value={newProd.barcode} onChange={(e) => setNewProd({ ...newProd, barcode: e.target.value })} className="field-input flex-1 rounded-xl px-3 py-2 text-sm" />
              <button type="button" onClick={() => setScannerTarget("new")} className="icon-btn rounded-xl px-3"><Icon name="ScanLine" size={18} /></button>
            </div>
